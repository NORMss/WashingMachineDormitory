...
- Показывает, кем занята машинка (имя или номер комнаты).
...
- Позволяет указать свое время в минутах.
...
По умолчанию PIN-код: `1234`. Измени его в файле `config.json` или через переменную окружения `STATUS_PIN`.
...
- `POST /api/status/occupy` - занять машинку
  - body: `{ "minutes": 60, "occupiedBy": "комната 417", "pin": "1234" }`
